...
class EFFA(nn.Module):
    def __init__(self, in_channels, kernel_size=3):
        ...
        # 注意：这里需要根据通道数自适应，或者保持原有的 Conv1d 逻辑
        self.conv = nn.Conv1d(1, 1, kernel_size=kernel_size, padding=(kernel_size - 1) // 2)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x, edge):
        # x: 待增强的特征 (例如时间融合特征)
        # edge: 引导特征 (例如当前帧的空间特征)
        
        # 1. 空间维度的引导增强
        edge_gate = self.sigmoid(edge * x + edge)
        x = edge_gate + x
        ...
        # 3. 最终融合
        return x * y.expand_as(x) + edge_gate
...
class Neck(nn.Module):
    def __init__(self, channels=[128,256,512] ,num_frame=5):
        ...
        self.effa_fusion = EFFA(in_channels=channels[0])
        self.conv_final = CSWF(channels[0], channels[0])

    def forward(self, feats):
        # 1. 空间分支聚合 (Spatial Branch)
        r_feat_spatial = torch.cat([feats[j] for j in range(self.num_frame-1)], dim=1)
        r_feat_spatial = self.conv_ref(r_feat_spatial)
        c_feat = self.conv_cur(r_feat_spatial * feats[-1])
        c_feat = self.conv_cr_mix(torch.cat([c_feat, feats[-1]], dim=1))
        
        # 2. 时间分支聚合 (Temporal Branch)
        r_feats_temporal = torch.cat([self.__getattr__("attn_%d"%i)(feats[i-1], feats[-1]) 
                                     for i in range(1, self.num_frame)], dim=1)
        r_feat = self.conv_gl_mix(r_feats_temporal)
        
        # 3. 核心改进：使用 EFFA 进行精炼
        # 将当前帧特征 c_feat 作为引导(edge)，去增强时间特征 r_feat
        refined_feat = self.effa_fusion(r_feat, c_feat)
        
        # 4. 最后通过 CSWF 输出
        out_feat = self.conv_final(refined_feat, c_feat)
        return [out_feat]

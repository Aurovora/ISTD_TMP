...
cocoGt_path         = 'D:/affair/college/ISTD/TMP/DAUB/annotations/DAUB_instances_test2017.json'
dataset_img_path    = 'D:/affair/college/ISTD/TMP/DAUB/'
...
def get_history_imgs(line, num=num_frame):
    # 1. 自动处理路径，获取目录和文件名
    # os.path.split 可以在任何系统上正确分离目录和文件名
    dir_path, file_name = os.path.split(line)
    
    # 2. 分离文件名和后缀（例如: '236', '.bmp'）
    # os.path.splitext 可以在任何系统上正确分离文件名和后缀
    name_part, file_type = os.path.splitext(file_name)
    
    # 3. 转换为整数索引
    try:
        index = int(name_part)
    except ValueError:
        # 如果转换失败，可能是文件名不是纯数字，或者您的文件命名不规范
        raise ValueError(f"文件名解析失败：无法将 '{name_part}' 转换为数字。请确保文件名是纯数字 (例如: 001.bmp)")

    # 4. 生成历史帧路径列表
    # range(index - num + 1, index + 1) 生成如 [232, 233, 234, 235, 236] 的索引
    history_paths = []
    for i in range(index - num + 1, index + 1):
        target_id = max(i, 0) # 确保索引不小于 0 (序列起点)
        # 重新组合：使用 os.path.join(目录, 文件名 + 后缀)
        # file_type 包含了 '.'，例如 '.bmp'
        target_file_name = f"{target_id}{file_type}" 
        p = os.path.join(dir_path, target_file_name)
...
if __name__ == "__main__":
    ...
    if 'info' not in cocoGt.dataset:
        cocoGt.dataset['info'] = {}  # 加一个空字典即可骗过 loadRes
